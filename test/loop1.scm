...
(deftest loop.1.20
  (let ((a 0) (b 5) (c 1))
    (loop for x from a to b by c
          collect (begin (incf a) (incf b 2) (incf c 3) x)))
  (0 1 2 3 4 5))
...
(deftest loop.1.29
  (let ((a #f)
        ...
        (i 0))
    (values
     (loop for x from (begin (set! a (incf i)) 0)
           below (begin (set! b (incf i)) 9)
           by (begin (set! c (incf i)) 2)
           collect x)
     a b c i))
  (0 2 4 6 8)
  1 2 3 3)
...
(deftest loop.1.40
  (loop for x from 1 to 5 do (begin) finally (return x))
  5)

;;; Test that the index variable acheives the exclusive
;;; upper bound, but does not exceed it.
(deftest loop.1.41
  (loop for x from 1 below 5 do (begin) finally (return x))
  4)

(deftest loop.1.42
  (loop for x from 10 downto 0 do (begin) finally (return x))
  0)

(deftest loop.1.43
  (loop for x from 10 above 0 do (begin) finally (return x))
  1)
...
(deftest loop.1.44
  (loop for i from 1 to 5 for c from 0+i collect c)
  (0+1i 1+1i 2+1i 3+1i 4+1i))

(deftest loop.1.45
  (loop for i from 1 to 5 for c from 0+1i by 2 collect c)
  (0+1i 2+1i 4+1i 6+1i 8+1i))

(deftest loop.1.46
  (loop for i from 1 to 5 for c downfrom 5+1i collect c)
  (5+1i 4+1i 3+1i 2+1i 1+1i))

(deftest loop.1.47
  (loop for i from 1 to 5 for c downfrom 10+1i by 2 collect c)
  (10+1i 8+1i 6+1i 4+1i 2+1i))

(deftest loop.1.48
  (loop for i from 1 to 5 for c upfrom 0+1i collect c)
  (0+1i 1+1i 2+1i 3+1i 4+1i))

(deftest loop.1.49
  (loop for i from 1 to 5 for c upfrom 0+1i by 2 collect c)
  (0+1i 2+1i 4+1i 6+1i 8+1i))
...
(deftest loop.1.50
  (let ((i 0))
    (loop for () from 10 to 15 collect (incf i)))
  (1 2 3 4 5 6))

(deftest loop.1.51
  (let ((i 0))
    (loop for () from 10 below 15 collect (incf i)))
  (1 2 3 4 5))

(deftest loop.1.52
  (loop for () from 10 to 0 collect 'a)
  ())

(deftest loop.1.53
  (let ((i 0))
    (loop for () from 0 to 10 by 2 collect (incf i)))
  (1 2 3 4 5 6))

(deftest loop.1.54
  (let ((i 0))
    (loop for () from 1 to 4
          for () from 1 to 10 collect (incf i)))
  (1 2 3 4))

(deftest loop.1.55
  (let ((i 0))
    (loop for () from 5 downto 0 collect (incf i)))
  (1 2 3 4 5 6))

(deftest loop.1.56
  (let ((i 0))
    (loop for () from 5 above 0 collect (incf i)))
  (1 2 3 4 5))
