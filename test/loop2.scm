...
(deftest loop.2.4
  (let ((x ()))
    (loop for e in '(a b c d) do (set! x (cons e x)))
    x)
  (d c b a))
...
(deftest loop.2.9
  (loop for (x () y) in '((a b c) (d e f) (g h i))
        collect (list x y))
  ((a c) (d f) (g i)))
...
;        (i 0))
;    (values
;     (loop for e in (begin (set! a (incf i))
;                           '(a b c d e f g))
;           by (begin (set! b (incf i)) cddr)
;           collect e)
;     a b i))
;  (a c e g)
;  1 2 2)
