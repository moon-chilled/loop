...
(deftest loop.9.6
  (loop for x from 1 to 10
        when (even? x) collect x into foo
        when (odd? x)  collect x into bar
...
(deftest loop.9.8
  (loop for x from 1 to 20
        when (eqv? (modulo x 5) 0) collect x into foo
        when (eqv? (modulo x 5) 2) collect x into foo
        finally (return foo))
  (2 5 7 10 12 15 17 20))

(deftest loop.9.9
  (loop for x from 1 to 20
        when (eqv? (modulo x 5) 0) collecting x into foo
        when (eqv? (modulo x 5) 2) collecting x into foo
        finally (return foo))
  (2 5 7 10 12 15 17 20))
...
(deftest loop.9.30
  (loop for x in '((a b) (c d) (e f g) () (i)) nconc (copy x))
  (a b c d e f g i))

(deftest loop.9.31
  (loop for x in '((a b) (c d) (e f g) () (i)) nconcing (copy x))
  (a b c d e f g i))
...
(deftest loop.9.34
  (loop for x in '(a b c d)
        nconc (list x)
        when (eq? x 'b) nconc (copy '(1 2 3))
        when (eq? x 'd) nconcing (copy '(4 5 6)))
  (a b 1 2 3 c d 4 5 6))
...
;   ((%m (z) z))
;   (loop for x in '((a b) (c d) (e f g) () (i))
;         nconc (expand-in-current-env (%m (copy x)))))
;  (a b c d e f g i))
